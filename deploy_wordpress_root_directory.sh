...
# Directorio de instalacion de WordPress
...
# 3. Solicitar detalles de la base de datos
...
# 4. Limpiar el directorio de instalacion (CUIDADO!)
echo "Limpiando el directorio $WP_ROOT_DIR..."
# Mueve los archivos preexistentes (como index.html de Apache) a una copia de seguridad
if [ -d "$WP_ROOT_DIR" ]; then
    mkdir -p /tmp/wp_backup_$(date +%Y%m%d%H%M%S)
    mv ${WP_ROOT_DIR}/* /tmp/wp_backup_$(date +%Y%m%d%H%M%S)/ 2>/dev/null
fi


# 5. Descargar y descomprimir WordPress
...
echo "Descomprimiendo archivos en $WP_ROOT_DIR..."
tar -xzf /tmp/latest.tar.gz -C /tmp/
# Mover el contenido del directorio 'wordpress' al directorio raiz de Apache
mv /tmp/wordpress/* $WP_ROOT_DIR/
...
cp ${WP_ROOT_DIR}/${WP_CONFIG_SAMPLE} ${WP_ROOT_DIR}/${WP_CONFIG_FILE}

# Reemplazar los marcadores de posicion de la base de datos
sed -i "s/database_name_here/$DB_NAME/g" ${WP_ROOT_DIR}/${WP_CONFIG_FILE}
sed -i "s/username_here/$DB_USER/g" ${WP_ROOT_DIR}/${WP_CONFIG_FILE}
sed -i "s/password_here/$DB_PASSWORD/g" ${WP_ROOT_DIR}/${WP_CONFIG_FILE}
...
sed -i "/define('AUTH_KEY',/r /dev/stdin" ${WP_ROOT_DIR}/${WP_CONFIG_FILE} <<< "$SALTS"
# Eliminar las lineas de salts originales/placeholder
sed -i "/put your unique phrase here/d" ${WP_ROOT_DIR}/${WP_CONFIG_FILE}


# 7. Asignar los permisos correctos
...
chown -R $APACHE_USER:$APACHE_GROUP $WP_ROOT_DIR
...
echo "Ahora puedes finalizar la instalacion visitando tu dominio o la IP del servidor."
echo "URL: http://tu_dominio_o_ip/"
